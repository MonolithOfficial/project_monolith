...
import narikala from '../audio/asm/1_alternative_dimension.mp3';
...
function CustomAudioPlayerASM() {
    ...
    const [currentTrack, setCurrentTrack] = useState("Alternative Dimension");
    const [trackState, setTrackState] = useState(`${translate("Paused")} - `);
    

    const query = useStaticQuery(graphql`
        query {
            wd: allFile(filter: {extension: {regex: "/(mp3)/"}, relativeDirectory: {eq: "asm"}}, sort: {fields: name}) {
            edges {
                node {
                id
                relativePath
            ...
            bg: file(relativePath: {eq: "a_system_message_cv_empt.png"}) {
                id
                childImageSharp {
                    fluid {
    ...
    const nameArray = ["Alternative Dimension (Intro)", "Kedeli", "A System Message", "Imperative", "Matarebeli", "Electrocuted",
    "Darkroom Laboratory", "Intermission (Outro)"];
...
export default CustomAudioPlayerASM;
