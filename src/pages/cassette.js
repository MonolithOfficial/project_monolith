...
const CassettePage = () => {
  ...
  const selectTrack = (index, autoPlay = true) => {
    ...
    console.log(`[CASSETTE] Track selected: ${list[index].name}`)
    stopRewinding()
  ...
  const handlePlay = () => {
    if (!activeCassette) { console.log("[CASSETTE] PLAY pressed — no cassette loaded!"); return }
    console.log("[CASSETTE] PLAY pressed — starting playback")
    stopRewinding()
    setIsRewinding(false)
  ...
  const handleStop = () => {
    console.log("[CASSETTE] STOP pressed — halting all transport")
    stopRewinding()
    setIsRewinding(false)
    setIsRecording(false)
  ...
  const handleRec = () => {
    if (!activeCassette) { console.log("[CASSETTE] REC pressed — no cassette loaded!"); return }
    console.log("[CASSETTE] REC pressed — recording engaged")
    stopRewinding()
    setIsRewinding(false)
    setIsRecording(true)
    audioRef.current?.pause()
  ...
  const handleRew = () => {
    if (!activeCassette) { console.log("[CASSETTE] REW pressed — no cassette loaded!"); return }
    console.log("[CASSETTE] REW pressed — rewinding tape")
    const audio = audioRef.current
    if (!audio) return
    setIsRecording(false)
    audio.pause()
  ...
  const handleLoad = () => {
    ...
    console.log(`[CASSETTE] LOAD pressed — cassette ${nextEjected ? "ejected" : "loaded"}`)
  ...
  const handleShelfCassette = (cassette) => {
    if (loadedCassette?.id === cassette.id && !isEjected) {
      console.log(`[CASSETTE] ${cassette.title} — already loaded in player`)
      return
    }
    console.log(`[CASSETTE] Loading "${cassette.title}" into player`)
    stopRewinding()
    const audio = audioRef.current
    if (audio) {
      audio.pause()
